...
pub enum Archive {
    ...
    SevenZ(PathBuf/*, SevenzArchive<File>*/, Option<String>),
    ZipMultipart(PathBuf, Option<String>)
}
...
impl Archive {
    ...
    /// Open an archive with an optional password (for encrypted zips)
    pub fn open_with_password<T: Into<PathBuf>>(
        path: T,
        password: Option<&str>
    ) -> anyhow::Result<Self> {
        ...
        if &path_str[path_str.len() - 4..] == ".zip" {
            ...
            Ok(Archive::SevenZ(path/*, SevenzArchive::open(path, &[])?*/, password.map(String::from)))
        }
            ...
            Ok(Archive::ZipMultipart(path, password.map(String::from)))
        }
    ...
    pub fn get_entries(&mut self) -> anyhow::Result<Vec<Entry>> {
        ...
        match self {
            ...
            Archive::SevenZ(path, password) |
            Archive::ZipMultipart(path, password) => {
                ...
                let mut list_cmd = Command::new(get7z()?);
                list_cmd.arg("l").arg(&path);

                if let Some(password) = password {
                    list_cmd.arg(format!("-p{password}"));
                }
    ...
    pub fn extract<T: Into<PathBuf> + std::fmt::Debug>(&mut self, folder: T) -> anyhow::Result<()> {
        ...
        match self {
            ...
            Archive::SevenZ(archive, password) |
            Archive::ZipMultipart(archive, password) => {
                ...
                // Extract the archive
                let mut extract_cmd = Command::new(get7z()?);
                extract_cmd
                    .arg("x")
                    .arg(archive)
