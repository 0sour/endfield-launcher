...
/// 
/// `$HOME/.local/share/arknights-launcher/sessions.json`
#[inline]
...
pub struct SessionData {
    // [Software\\HyperGryph\\Arknights]
    pub game_reg: String,

    // [Software\\HyperGryphSDK]
    pub sdk_reg: String
}
...
impl SessionsExt for Sessions {
    ...
    fn update(name: String, prefix: impl AsRef<Path>) -> anyhow::Result<()> {
        ...
        for entry in std::fs::read_to_string(prefix.as_ref().join("user.reg"))?.split("\n\n") {
            if entry.starts_with("[Software\\\\HyperGryph\\\\Arknights]") {
                new_session.game_reg = entry.to_owned();
            }

            else if entry.starts_with("[Software\\\\HyperGryphSDK]") {
    ...
    fn apply(name: String, prefix: impl AsRef<Path>) -> anyhow::Result<()> {
        ...
        let entries: String = std::fs::read_to_string(prefix.as_ref().join("user.reg"))?
            .split("\n\n")
            .map(|entry| {
                let new_entry = if entry.starts_with("[Software\\\\HyperGryph\\\\Arknights]") {
